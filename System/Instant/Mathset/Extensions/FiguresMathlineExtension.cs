namespace System.Instant
{
    ...
    public static class FiguresMathsetExtension
    {
        ...
        {
            IComputation[] ic = rubrics
                .Select(
                    r =>
                        figures.Computations
                            .Where(c => ((Computation)c).ContainsFirst(r))
                            .FirstOrDefault()
                )
                .Where(c => c != null)
                .ToArray();
        ...
        {
            IComputation ic = figures.Computations
                .Where(c => ((Computation)c).ContainsFirst(rubric))
                .FirstOrDefault();
            if (ic != null)
                ic.Compute();
            return figures;
